...
class LossHistory(keras.callbacks.Callback):
    def __init__(self, training_data, model, batch_size, total_epoch, save_dir, determine_threshold=5,
                 satisfied_acc=0.7,
                 checktype='epoch_5', satisfied_count=3, pkl_dir=None, solution=None,
        ...
        self.log_file.write(
            '{},{},{}\n'.format('checktype', 'current_epoch', 'time_usage'))

        self.feature_log_file.write(','.join(self.Monitor.get_features().keys()) + ',loss,acc,val_loss,val_acc\n')
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        if self.checktype == 'batch' and batch % self.checkgap == 0:
            ...
            self.log_file.write('{},{},{}\n'.format(self.checktype, batch, str(time.time() - self.start_time)))
            self.log_file.flush()
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch % self.checkgap == 0 and self.checktype == 'epoch':
            ...
            self.evaluated_gradients = None
            # columns_name = list(self.feature_list.keys()).extend(['loss','acc','val_loss','val_acc'])
            dictwriter_object = csv.DictWriter(self.feature_log_file, fieldnames=self.feature_list.keys())
            dictwriter_object.writerow(self.feature_list)
            self.feature_log_file.flush()

            self.log_file.write('{},{},{}\n'.format(self.checktype, epoch, str(time.time() - self.start_time)))
            self.log_file.flush()
